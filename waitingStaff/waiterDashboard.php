<?php
    ...
    if(isset($_POST['served']))
    {
        ...
        $sql="update orderList set `STATUS`=\"served\" WHERE tabel=$tableNum AND itemID=\"$ItemId\" " ;
    ...
    <div class="list-group" id="items-list">
        ...
        <?php 
            ...
            $comp=new mainMenuComponent();
                ...
                $tableNum=3;
                $result=$dataBase->getWaiterOrderedList();
                if($result!=null){
                    while($row=mysqli_fetch_assoc($result)){
                        $comp->waiterOrdersItemComponent($row["itemName"],$row["itemID"],$row["quantity"],$row["tabel"]);
                    }
...
<script>
  ...
  refresh.addEventListener("click",function(){
    ...
    httpReq.open('GET','http://localhost:8080/smartRestaurant/waitingStaff/getReadyItems.php',true);
    ...
    var tableRow='<div class="list-group-item" style="background-color:#ce1e1e;">\
       ...
       if(data.lenght>0){
            for(i=0;i<data.length;i++){
                tableRow +='<form method="POST" action=""><div class="list-group-item"><h4 id="item-name">'+data[i].itemName+'</h4><h4 id="item-qty">'+data[i].quantity+'</h4><h4 id="item-tableNum">'+data[i].tabel+'</h4><button id="btn-addCart" name="served" >served</button><div style="clear:both"></div><input type="hidden" name="itemID" value="'+data[i].itemID+'"><input type="hidden" name="tableNum" value="'+data[i].tabel+'"></div></form>';
                    
            }
       }else{
           tableRow+='<h1><center>No Items Ordered</center></h1>';
       }
